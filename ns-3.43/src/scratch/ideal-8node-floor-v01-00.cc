...
main (int argc, char *argv[])
{
  ...
  // NOTE: Anda menggunakan ConstantPositionMobilityModel (Diam).
  // Jika ingin menguji LET (Link Expiration Time) yang bergantung kecepatan, 
  // Anda mungkin perlu mengubah ini menjadi ConstantVelocityMobilityModel di masa depan.
  ...
  // --- BAGIAN INI DITAMBAHKAN UNTUK MENGHITUNG STATISTIK PAKET ---
  flowMon->CheckForLostPackets (); // Periksa paket yang mungkin masih "terbang"
  ...
  std::cout << "\n-----------------------------------------------------------" << std::endl;
  std::cout << "               STATISTIK ALIRAN PAKET (FLOW)" << std::endl;
  std::cout << "-----------------------------------------------------------" << std::endl;

  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
      // Menjumlahkan statistik dari setiap aliran (flow)
      totalTxPackets += i->second.txPackets;
      totalRxPackets += i->second.rxPackets;
      // Paket Hilang = Dikirim - Diterima
  ...
  if (totalTxPackets > 0)
    {
      pdr = ((double)totalRxPackets / (double)totalTxPackets) * 100.0;
    }

  std::cout << "Total Paket Dikirim (Tx)   : " << totalTxPackets << std::endl;
  std::cout << "Total Paket Diterima (Rx)  : " << totalRxPackets << std::endl;
  std::cout << "Total Paket Hilang (Lost)  : " << totalLostPackets << std::endl;
